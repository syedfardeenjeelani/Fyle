...
contactUsBtn.addEventListener("click", () => {
  // Ensure the modal is initially hidden with display none
...
form.addEventListener("submit", async function (e) {
  ...
  try {
    const response = await fetch("https://getform.io/f/raeqdgqa", {
      method: "POST",
      body: formData,
      headers: {
    ...
    if (response.ok) {
      ...
      toast.classList.add('sshow') 
      toast.classList.add('success')
      setTimeout(() => {
        toast.classList.remove('sshow') 
    ...
    toast.classList.add('sshow') 
    toast.innerHTML = 'WOMP WOMP LIMIT EXCEEDED '
...
if(!isPortrait){
  autoSlideInterval = setInterval(nextImg, 1850);
}

slider.addEventListener('mouseenter', () => {
  clearInterval(autoSlideInterval);
  console.log('Animation paused, interval cleared');
});

slider.addEventListener('mouseleave', () => {
  if(!isPortrait){
  autoSlideInterval = setInterval(nextImg, 1700);
}
  console.log('Animation resumed, interval restarted');
});
...
//     event.target !== iagree &&
//     event.target !== iagreeLabel &&
//     event.target !== formContactUsBtn
//   ) {
//     modal.style.display = "none";
//     popUP = false;
//   }
